...
class FrozenActorAsBehavior(nn.Module):
...
class GFlow_W2:
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            if self.use_natgrad:
                nat_scale = torch.exp(-2.0*log_std_a.detach())
                nat_scale = nat_scale.clamp(min=1e-3, max=1e3)
                h = log_std_a.register_hook(lambda g: g * nat_scale)  # G_{ρρ}^{-1} = 0.5 I
                actor_loss.backward()
                h.remove()
            else:
                actor_loss.backward()
            ...
            metrics.update({
                ...
                "actor_std": std_a.detach().mean().cpu().item(),
                "actor_mean": mean_a.detach().mean().cpu().item(),
                "behavior_std": std_b.detach().mean().cpu().item(),
                "behavior_mean":mean_b.detach().mean().cpu().item(),
            })
...
class GFlow_W2_Refine:
    ...
    def select_action(self, state, deterministic = True):
        ...
        if deterministic:
            ...
            return torch.clamp(mean.cpu().data.numpy().flatten() + torch.randn_like(mean) * torch.exp(log_std), -self.max_action, self.max_action)
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        # ----- Behavior (1-step) -----
        metrics = {"critic_loss": 0.0, "behavior_loss": 0.0}
