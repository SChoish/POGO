...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_natgrad", action="store_true", help="Use Natural Gradient")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./models_mid", help="Checkpoint directory")
    ...
    MID_DIR = args.checkpoint_dir
    os.makedirs(MID_DIR, exist_ok=True)
    midpoint = int(round(args.max_timesteps * args.split_ratio))
    ...
    # start_mode='load'일 때 체크포인트 로드
    if args.start_mode == "load" and args.load_prefix != "":
        print(f"[LOAD] Loading checkpoint from: {args.load_prefix}")
        agentA.load(args.load_prefix)
    ...
    # start_mode가 'load'이면 Phase-1 스킵
    if args.start_mode == 'load':
        print(f"⏭️  Phase-1 스킵 (체크포인트에서 로드)")
        t = split_step - 1  # Phase-1 완료 상태로 설정
        midpoint_saved = True
    else:
        for t in range(split_step):
            ...
            # midpoint에서 체크포인트 저장
            if (t + 1) == midpoint and not midpoint_saved:
    ...
    # 루프 종료 후에도 midpoint에서 저장하지 못했다면 마지막에 저장
    if not midpoint_saved:
        mid_name = f"{file_name}_mid_{t+1}"
        agentA.save(f"{MID_DIR}/{mid_name}")
        print(f"[CKPT] Saved final checkpoint at step {t+1} -> {MID_DIR}/{mid_name}_*")
    if args.wandb:
        wandb.log({"ckpt/mid_saved_step": t + 1}, step=t + 1)
